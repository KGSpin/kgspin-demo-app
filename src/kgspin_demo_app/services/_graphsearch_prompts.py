"""Vendored prompts for the paper-mirror GraphSearch pipeline.

PRD-004 v5 Phase 5A — deliverable E. Prompts in this module are
vendored verbatim from RAGSearch / GraphSearch:

    Repo:    https://github.com/HKUST-DSAIL/RAGSearch
    Path:    GraphSearch/deepsearch/prompts.py
    Paper:   "GraphSearch: Decomposed and Verifiable Graph Reasoning
              for Question Answering on Knowledge Graphs"
              arXiv:2509.22009
    License: Apache 2.0 (see RAGSearch/GraphSearch/LICENSE)

The dual-channel (text + KG) iterative-decomposition behavior of
``services/graphsearch_pipeline.py`` is meaningful only because each
prompt below is faithful to the original. We do not modify them in
Phase 5A; any tuning belongs in a separate PR with an attribution
diff.

If the paper's prompts.py is updated upstream (the GraphSearch repo
continues to evolve), re-syncing is a 5-minute manual step:

    1. ``cd /path/to/RAGSearch/GraphSearch/deepsearch``
    2. ``diff prompts.py kgspin-demo-app/src/.../`_graphsearch_prompts.py``
    3. Apply non-breaking changes (typos, additional examples).
    4. Bump the attribution comment with the upstream commit hash.

Do NOT paraphrase to avoid attribution. Apache 2.0 requires the
verbatim license + copyright preservation; vendoring with attribution
satisfies that.
"""
